...
class SqlBuilder
{
    ...
    private function compileCount() {
        $components = array('from', 'joins', 'wheres', 'groupBy');
    ...
    private function compileJoin($join) {
        return strtoupper($join->type) . ' JOIN ' . $this->quoteIdentifier($join->table) . str_replace('WHERE', ' ON', $this->compileWheres($join->clauses));
    }
